...
export async function generateMetadata({ params }: PageProps<'/produtos/[slug]'>) {
  ...
  if (!product) return { title: 'Produto não encontrado — Fast Commerce' };

  return {
    title: `${product.name} — Fast Commerce`,
...
export default async function Product({ params }: PageProps<'/produtos/[slug]'>) {
  ...
  const [department, subcategory] = await Promise.all([
    findCategory(product.department),
    findCategory(product.subcategory),
  ]);
  ...
  return (
    <main className="page-shell flex flex-1 flex-col gap-10 py-10">
      <nav aria-label="Trilha" className="flex flex-wrap items-center gap-1 text-sm text-zinc-500">
        ...
        <Link href={`/categorias/${product.department}`} className="hover:underline">
          {department?.name ?? product.department}
        </Link>
        <span aria-hidden="true">/</span>
        <Link href={`/categorias/${product.subcategory}`} className="hover:underline">
      ...
      <div className="grid gap-10 lg:grid-cols-2">
        <div className="flex flex-col gap-3">
          <Image
            src={product.images[0] ?? product.thumb_image}
            alt={product.name}
            width={600}
            height={600}
        ...
        <div className="flex flex-col gap-6">
          <header className="flex flex-col gap-2">
            <p className="text-sm text-zinc-500 uppercase">{product.brand}</p>
          ...
          <SkuSelector items={product.items} optionType={product.optionType} />
          ...
          <section className="flex flex-col gap-2 border-t border-black/10 pt-6 dark:border-white/15">
            ...
            <dl className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1 text-sm text-zinc-600 dark:text-zinc-400">
              ...
              <dt>Variações</dt>
              <dd>
                {product.items.length} por {product.optionType}
              </dd>
              <dt>Código</dt>
              <dd>{product.id}</dd>
            </dl>
